import { CardInterface } from "../logic/interfaces";

export const MinutesCard = ({ number, setNumber, children }: CardInterface) => {
  return (
    <div className={`flex flex-col items-center`}>
      <button
        ...
        id="decrement-button"
        onClick={() => {
          setNumber((number) => (number == 0 ? 0 : number - 1));
          console.log(`VALOR DE MINUTOS DECREMENTADO ${number}`);
        }}
      ...
      <input
        ...
        placeholder="00"
        value={number}
        onChange={(e) => setNumber(parseInt(e.target.value, 10) || 0)} // Asegura que el valor sea un número válido
        required
      />
      <button
        type="button"
        id="increment-button"
        onClick={() => {
          setNumber((number) => (number == 59 ? 59 : number + 1));
          console.log(`VALOR DE MINUTOS INCREMENTADO ${number}`);
        }}
